...
const unsigned long DOUBLE_PRESS_MS = 800;  // Increased from 400ms to 800ms for easier use
...
void handleButton() {
    ...
    if (btnReading != btnState) {
      ...
      if (btnState && !buttonPressed) { // Just pressed
        ...
        if (pressDuration >= LONG_PRESS_MS) {
          ...
          // Show immediate feedback on first click
          if (clickCount == 1 && doseState == IDLE) {
            Serial.println("First click - showing preset change immediately");
            cyclePresets();
          }
        }
  ...
  if (clickCount > 0 && (now - lastClickTime > DOUBLE_PRESS_MS)) {
    if (clickCount == 1) {
      Serial.println("Single click timeout - already handled");
      // Single click already handled immediately above
...
void handleShortPress() {
  ...
  Serial.println("Short press detected (delayed handler)");
  
  switch (doseState) {
    case IDLE:
      // Single press already handled immediately - do nothing
      Serial.println("Single press already handled immediately");
      break;
    case MENU:
      handleMenuSelect();
      break;
...
void handleDoublePress() {
  menuLastAction = millis();
  Serial.print("Double press detected in state: ");
  Serial.println(doseState);
  
  switch (doseState) {
    case IDLE:
      Serial.println("Calling startDoseSequence()");
      // Double press = Start dosing
      startDoseSequence();
      break;
    default:
      Serial.println("Calling exitToIdle()");
...
void showIdleScreen() {
  ...
  display.print("Press=Preset 2x=Dose");
