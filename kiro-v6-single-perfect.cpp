...
const unsigned long PRESET_PRESS_MS = 2000;  // 2 seconds for preset select
const unsigned long MENU_PRESS_MS = 5000;    // 5 seconds for menu
...
bool checkForAdvancedStall(float weight, unsigned long now) {
  // TEMPORARILY DISABLED - Unjam feature causing issues
  // TODO: Implement proper stall detection and unjam sequence
  // See .kiro/specs/smart-unjam-system/requirements.md for full specification
  // 
  // Implementation tasks:
  // 1. Add proper state isolation (only during dosing)
  // 2. Implement weight-based flow detection with configurable sensitivity
  // 3. Add progressive unjam sequence with increasing intensity
  // 4. Implement attempt limiting and safety timeouts
  // 5. Add user feedback and status display
  // 6. Create configuration interface for tuning parameters
  // 7. Add comprehensive testing and validation
  return false;
}

void performAdvancedUnjam() {
  // TEMPORARILY DISABLED - Function stub for future implementation
  // TODO: Implement according to .kiro/specs/smart-unjam-system/requirements.md
  Serial.println("Unjam feature temporarily disabled - see spec for future implementation");
}
...
void handleButton() {
  ...
  if ((now - lastBtnChange) > DEBOUNCE_MS) {
    if (btnReading != btnState) {
      ...
      if (btnState && !buttonPressed) { // Just pressed
        pressStartTime = now;
        buttonPressed = true;
        presetShown = false;
        menuShown = false;
      ...
      } else if (!btnState && buttonPressed) { // Just released
        buttonPressed = false;
        ...
        Serial.print("Checking press duration: ");
        Serial.print(pressDuration);
        Serial.print("ms (Menu threshold: ");
        Serial.print(MENU_PRESS_MS);
        Serial.print("ms, Preset threshold: ");
        Serial.print(PRESET_PRESS_MS);
        Serial.println("ms)");
        
        if (pressDuration >= MENU_PRESS_MS) {
          // 5+ seconds = Enter menu
          Serial.println("Duration >= MENU_PRESS_MS - calling handleMenuPress()");
          handleMenuPress();
        } else if (pressDuration >= PRESET_PRESS_MS) {
          // 2-5 seconds = Cycle presets
          Serial.println("Duration >= PRESET_PRESS_MS - calling handlePresetPress()");
          handlePresetPress();
        } else {
          // Short press = Start dosing
          Serial.println("Short duration - calling handleShortPress()");
          handleShortPress();
        }
    ...
    // Show feedback while button is held
    if (buttonPressed) {
      unsigned long holdDuration = now - pressStartTime;
      
      if (holdDuration >= MENU_PRESS_MS && !menuShown) {
        showMessage("Release for\nMENU");
        menuShown = true;
      } else if (holdDuration >= PRESET_PRESS_MS && !presetShown && !menuShown) {
        showMessage("Release for\nPRESET");
        presetShown = true;
      }
    }
...
void handleShortPress() {
  ...
  Serial.println("Short press handler - Start dosing");
...
void handlePresetPress() {
  menuLastAction = millis();
  Serial.println("Preset press handler - Cycle presets");
  
  switch (doseState) {
    case IDLE:
      // 2 second press = Cycle presets
      cyclePresets();
      break;
    default:
      // In other states, exit to idle
      exitToIdle();
      break;
  }
}
...
void handleMenuPress() {
  menuLastAction = millis();
  Serial.println("=== Menu press handler called ===");
  Serial.print("Current doseState: ");
  Serial.println(doseState);
  
  switch (doseState) {
    case IDLE:
      Serial.println("Calling enterMainMenu()");
      // 5 second press = Enter menu
      enterMainMenu();
      break;
    case MENU:
    case STATISTICS:
...
// ---------------------------------------------------------------
void enterMainMenu() {
  Serial.println("=== enterMainMenu() called ===");
  doseState = MENU;
  menuItem = 0;
  inSubMenu = false;
  Serial.println("State set to MENU, calling showMainMenu()");
  showMainMenu();
  Serial.println("=== enterMainMenu() completed ===");
}

void showMainMenu() {
  Serial.println("=== showMainMenu() called ===");
  ...
  Serial.print("Displaying menu item ");
  ...
  display.display();
  Serial.println("=== showMainMenu() display updated ===");
}
...
void showIdleScreen() {
  ...
  display.print("Tap=Dose Hold=Preset");
...
// ---------------------------------------------------------------
void cyclePresets() {
  Serial.println("=== cyclePresets() called ===");
  ...
  // Show brief preset change message
  display.clearDisplay();
  display.setTextSize(1);
  display.setCursor(0, 8);
  display.print("Preset: ");
  display.println(presets[activePreset].name);
  ...
  showIdleScreen();
  Serial.println("=== cyclePresets() completed ===");
}
...
void stopMotor() {
  // Stop any pending movements
  stepper.stop();
  
  // Wait a moment for deceleration
  delay(10);
  
  // Disable motor power
  motorOff();
  
  // Reset stepper position to prevent accumulated errors
...
void exitToIdle() { 
  ...
  Serial.println("Exited to IDLE - all systems reset");
}
void updateStatistics() { /* Statistics updated in learning function */ }
void resetToIdleState() {
  // Comprehensive reset when entering IDLE
  stopMotor();
  unjamCount = 0;
  doseStartTime = 0;
  phaseStartTime = 0;
